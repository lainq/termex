...
export const getTermexSettings = (): TermexSettings => {
  ...
  if (!existsSync(filename)) {
    writeFileSync(
      filename,
      Buffer.from(JSON.stringify(defaultSettings), "utf8").toString("base64")
    );
    return defaultSettings;
  }
  const settings: TermexSettings = JSON.parse(
    Buffer.from(readFileSync(filename).toString(), "base64").toString("utf8")
  );
  return settings;
};

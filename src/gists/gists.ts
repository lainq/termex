...
export class GithubGist {
    ...
    private incrementFileIndex = (by:number, limit:Array<any>):void => {
        if(this.fileIndex == -1 || this.fileIndex == limit.length-1){return}
        this.fileIndex += by
    ...
    private addKeys = (gists:Array<any>):void => {
        if(this.addedInputs) {return}
        const input = new InputMode(new Map<string, Function>([
            ['up', () => {this.incrementFileIndex(1, gists)}],
            ['down', () => {this.incrementFileIndex(-1, gists)}]
        ]))

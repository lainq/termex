...
export const openLast = () => {
    const history:Array<HistoryObject> = TermexHistory.readFromHistory().map((element:string):HistoryObject => {
    ...
    const exists:boolean = checkFileExists(lastElement.filename, lastElement.isDirectory)

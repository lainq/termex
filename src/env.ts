...
class EnvFileParser {
    ...
    public env = ():Map<string, string> => {
        ...
        const variables:Map<string, string> = this.createEnvironmentVariables(fileContent.toString().split("\n"))
        return variables
    }

    private createEnvironmentVariables = (lines:Array<string>):Map<string, string> => {
        let parseLines:Array<string> = lines.filter((currentLine:string):boolean => {
        ...
        for(let index=0; index<parseLines.length; index++){
            ...
            const split = currentLine.split("=").filter((element:string):boolean => {
    ...
    public static createMapString = <Key, Value>(data:Map<Key, Value>, joinCharacter:string):string => {
...
export class EnvironmentVariables {
    ...
    private createEnvString = (files:Array<string>):string => {
        ...
        for(let index=0; index<files.length; index++){
            ...
            if(variables.size > 0){
                envString += EnvFileParser.createMapString<string, string>(variables, '\n')
            }
        }
        return envString
    }

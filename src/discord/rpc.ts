...
export class RichPresenceSettings {
    public static readonly settingsFile:string = join(homedir(), ".termex", ".rpc")

    public static getClientId = ():string | null => {
        if(!checkFileExists(RichPresenceSettings.settingsFile, false)) {
            return null
        }
        const key:string = readFileSync(RichPresenceSettings.settingsFile).toString().trim()
...
export class TermexDiscordRPC {
    ...
    public createRichPresence = (clientId:string, filename:string, timeout:number=15e3):void => {

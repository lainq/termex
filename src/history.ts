import { cyanBright, greenBright, magenta, magentaBright, yellowBright } from "chalk";
...
export class TermexHistory {
    private static readonly filename:string = join(homedir(), ".termex", "history")
...
export const displayHistory = ():void => {
    const data:Array<HistoryObject> = TermexHistory.readFromHistory().map((element:string):HistoryObject => {
    ...
    for(let index=0; index<data.length; index++){
        ...
        tableData.push([
            ...
            checkFileExists(currentObject.filename, currentObject.isDirectory).toString(),

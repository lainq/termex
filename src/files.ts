...
export class DirectoryFiles {
  ...
  private generateMap = (files: Array<string>) => {
    let generatedMap: Map<string, Array<string>> = new Map<
      string,
      Array<string>
    >();
    for (let index = 0; index < files.length; index++) {
      ...
      baseDirectory =
        baseDirectory == this.path
          ? "."
          : baseDirectory.slice(this.path.length);
  ...
  private displayTree = (generatedMap: Map<string, Array<string>>): void => {
    ...
    for (let index = 0; index < keys.length; index++) {
      ...
      outputString +=
        greenBright(keys[index]) +
        " => " +
        yellowBright(element.toString()) +
        "\n";
    }
